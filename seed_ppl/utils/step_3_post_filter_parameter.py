...
def filter_by_prompt_eval_score(data):
    ...
    maintain_data = [entry for entry in data if entry['quality_score']>8 and entry['following_score']>8]
    # maintain_data = [entry for entry in maintain_data if entry['following_score']> 6]
    
    filtered_data = [entry for entry in data if entry['quality_score']<9 or entry['following_score']<9]
