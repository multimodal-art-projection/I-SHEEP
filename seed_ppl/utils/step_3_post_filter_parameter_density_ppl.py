...
def filter_by_density(data):
    print("Filtering by cluster density...")
    print(f"Original data length is: {len(data)}")
    
    # 创建一个字典，将数据按 cluster_id 分组
    cluster_dict = defaultdict(list)
    for entry in data:
        cluster_dict[entry['cluster_id']].append(entry)
    
    # 随机从每个 cluster 中选择一条数据组成 maintain_data，其余的作为 filtered_data
    maintain_data = []
    filtered_data = []
    
    for cluster_id, entries in cluster_dict.items():
        # selected_entry = random.choice(entries)
        selected_entry = min(entries, key=lambda x: x['pair_ppl'])
        maintain_data.append(selected_entry)
        filtered_data.extend([entry for entry in entries if entry != selected_entry])
    
    print(f"The maintain data length is: {len(maintain_data)}")
    print(f"The filtered data length is: {len(filtered_data)}")
...
#     maintain_data = [entry for entry in data if entry['output_ppl']<50 and entry['pair_ppl']<50]
#     # maintain_data = [entry for entry in maintain_data if entry['following_score']> 6]
    
#     filtered_data = [entry for entry in data if entry['output_ppl']>50 or entry['pair_ppl']>50]
...
def main(data_path, targetfp):
    ...
    maintain_data, filtered_data = filter_by_density(total_data)
